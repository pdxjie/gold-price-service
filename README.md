...
仓库根目录提供了一组便捷脚本，推荐直接使用：

```bash
./desktop.sh
```

脚本会进入内层 `gold-price-service/` 项目，读取 `.nvmrc`，自动安装依赖，并启动 Electron 桌面浮窗。Electron 会检查 `http://localhost:3001`，如果后端没有运行，会自动拉起本地后端。
...
```text
gold-price-service/data/gold-prices.sqlite
```
...
数据库中保存实时价格采样、黄金回收价、提醒规则、提醒事件、通知配置、大模型配置和 AI 分析记录。
